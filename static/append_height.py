...
writeFile.write("OFF\n")
writeFile.write(str(vertex_count) + ' ' + str(face_count) +'\n')

for i in range(vertex_count):
    print(str(vertex_list[i][2]))
    writeFile.write( str(vertex_list[i][0]) + ' ' + str(vertex_list[i][1]) + ' ' + str(vertex_list[i][2]) + ' ' + str(vertex_list[i][1]) + '\n')

for i in range(face_count):
    v_connected = face_temp_list[i].split(' ')
    writeFile.write("3 " + v_connected[1] + ' ' + v_connected[2] + ' ' + v_connected[3])